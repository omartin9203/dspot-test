...
export abstract class BaseError extends Error implements IResultError {
  ...
  constructor({ name, message, internationalization }: BaseErrorProps) {
  ...
  protected static translateErrorMessage(
    code: string,
    internationalization: InternationalizationType,
    message?: string,
    lan?: string
  ): string {
    lan = !lan || !Object.keys(internationalization.messagesProvider).includes(lan) ? 'en' : lan;
    message =
      internationalization.messagesProvider[lan][code] ??
      message ??
      `Does not exist message provider for ErrorCode: [${code}] and Language: [${lan}]`;
    if (!Object.keys(internationalization.messagesProvider[lan]).includes(code)) return message;
